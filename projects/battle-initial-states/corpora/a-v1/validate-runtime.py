...
def main():
    ...
    manifest={'model':'untrained APathActorCritic','model_seed':190919,'optimizer_updates':0,
              ...
              'full_episode_selection':'greedy source-independent coverage of card/encounter/relic/potion labels, max 32 states',
    ...
    (PROJECT/'runtime-manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
    ...
    report={'schema':'initial-state-runtime-diagnostic-v1','backend_sha256':actual,
            ...
            'scope':'Engineering diagnostics of source-rule-derived candidates; not historical equivalence, training or generalization',
            'hashes':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [
                Path(__file__),PROJECT/'initial_state.py',PROJECT/'pilot-states.jsonl',PROJECT/'runtime-manifest.json']}}
    (PROJECT/'runtime-report.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
