"""Resume an interrupted critic run with strict restore and a canonical TB prefix."""
import argparse
import copy
import hashlib
import json
...
def main():
    ...
    parent_status=json.loads((parent/'status.json').read_bytes())
    previous_elapsed=float(parent_status['elapsed_seconds'])
    if (parent/'resource-telemetry.jsonl').exists():
        for line in (parent/'resource-telemetry.jsonl').read_text().splitlines():
            try:previous_elapsed=max(previous_elapsed,float(json.loads(line)['elapsed_seconds']))
            except (ValueError,KeyError):pass
    checkpoint_path=parent/'latest.pt'
    checkpoint_hash=hashlib.sha256(checkpoint_path.read_bytes()).hexdigest()
    saved=torch.load(checkpoint_path,map_location='cpu',weights_only=False)
    resume_step=saved['env_steps'];resume_update=saved['iteration']
    if saved['config']['architecture']!=args.architecture or not saved['resume_allowed'] or saved['phase']!='idle':
        raise ValueError('Checkpoint is not a resumable matching architecture')
    if saved.get('pending_rollout') is not None:raise ValueError('Incomplete update checkpoint')
    expected_fp=saved['fingerprint']
    del saved
    started=time.monotonic();deadline=started+args.seconds-previous_elapsed;train_deadline=deadline-3600
    if time.monotonic()>=train_deadline:raise ValueError('No original training time budget remains')
    write(out/'resume-provenance.json',dict(parent=str(parent),checkpoint_sha256=checkpoint_hash,
        resume_update=resume_update,resume_step=resume_step,previous_active_seconds=previous_elapsed,
        abandoned_logged_updates=parent_status['iteration']-resume_update,
        resume_script_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()))
    shutil.copy2(__file__,out/'resume-source.py')
    ...
    original_events=EventAccumulator(str(parent/'tensorboard'/(args.architecture+'-v1')),size_guidance={'scalars':0})
    original_events.Reload()
    for tag in original_events.Tags()['scalars']:
        for event in original_events.Scalars(tag):
            if event.step<=resume_step:
                writer.add_scalar(tag,event.value,event.step,walltime=event.wall_time)
    # Preserve the canonical metric prefix; old orphaned update records stay in the parent.
    with (out/'metrics.jsonl').open('w') as target:
        for line in (parent/'metrics.jsonl').read_text().splitlines():
            if json.loads(line)['iteration']<=resume_update:target.write(line+'\n')
    for name in ('initial.pt','selected.pt'):
        shutil.copy2(parent/name,out/name)
    for path in parent.glob('dev-*-summary.json'):
        update=int(path.name.split('-')[1])
        if update<=resume_update:
            shutil.copy2(path,out/path.name)
            episode_path=path.with_name(path.name.replace('-summary.json','-episodes.jsonl'))
            shutil.copy2(episode_path,out/episode_path.name)
    ...
    if fp!=expected_fp:raise ValueError('Frozen parent fingerprint mismatch')
    ...
    status=dict(status='starting',pid=os.getpid(),started_at=datetime.now(timezone.utc).isoformat(),
                env_steps=resume_step,iteration=resume_update,budget_seconds=args.seconds,formal_training_started=True,
                device=torch.cuda.get_device_name(0),fingerprint=fp,best_dev=None)
    trainer=None;best_score=parent_status['best_dev']['score'];best_update=parent_status['best_dev']['update'];best_path=out/'selected.pt'
    status['best_dev']=dict(update=best_update,score=best_score)
    ...
    def sample_resources():
        while not stop_telemetry.is_set():
            ...
            sample.update(wall_time=time.time(),elapsed_seconds=previous_elapsed+time.monotonic()-started)
    ...
    def refresh(phase,**extra):
        ...
        status.update(status=phase,elapsed_seconds=previous_elapsed+time.monotonic()-started,**extra)
        write(out/'status.json',status)
        step=trainer.env_steps if trainer is not None else resume_step
    ...
    try:
        refresh('restoring')
        trainer=APathTrainer.load(checkpoint_path)
        if trainer.env_steps!=resume_step or trainer.iteration!=resume_update:raise ValueError('Restore boundary differs')
        trainer.save(out/'latest.pt')
        status['active_environments_replayed']=True
        if resume_update>0 and resume_update%64==0 and not (out/f'dev-{resume_update:04d}-summary.json').exists():
            trainer.save(out/f'update-{resume_update:04d}.pt')
            eval_checkpoint(trainer.model,'audit-dev',f'dev-{resume_update:04d}',resume_step)
        refresh('training')
        last_duration=0.
        while trainer.env_steps<262144 and time.monotonic()+max(60,last_duration*2)<train_deadline:
            ...
            trainer.save(out/'latest.pt')  # Atomic idle-boundary recovery after every completed update.
        ...
        if hashlib.sha256(checkpoint_path.read_bytes()).hexdigest()!=checkpoint_hash:raise ValueError('Parent checkpoint changed')
