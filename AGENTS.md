...
独立worktree已完成75类150版本工程准入，最终938项回归、350局集成及64步全卡输入模型更新恢复通过。当前输入v3/模型v2，旧阶段数字为历史记录。资源及未来范围见docs/ironclad-full-cards-report.md，禁止将该工程验收当作正式PPO成绩或跨新敌人药水泛化证据。

## 2026-09-13 最终修复复核

当前CARD115、接口v4/模型v4；公开结算上下文同时进入actor与critic。完整950项回归通过。最新合并结论及证据以docs/card-token-final-review.md为准，前文旧模型与测试数字保留历史身份。未执行正式训练或合并。
