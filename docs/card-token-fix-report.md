...
### B03：Searing Blow +100

用户裁定是不围绕+100扩容，也不把它擅自定义成正版规则上的不可升级终点。当前处理为：

- `third_party/sts_lightspeed/src/combat/CardInstance.cpp:56-60`：`canUpgrade`在`specialData>=100`时返回false。
- `CardInstance.cpp:147`的资源保护仍保留；若绕过过滤直接越过边界，仍明确抛出资源异常。
- `tests/test_card_token_fixes.py::test_b03_searing_blow_plus_100_is_not_upgraded_by_armaments_plus`验证Armaments+1不再把+100送入升级异常。

+0至+100仍可构造，公式和编码不改；+100之后的资源边界记录为已知工程限制，不是本轮阻塞。
...
当前CARD token、PLAYER_GLOBAL token、选择种类、候选实体、合法mask和模型外路由能表达候选是谁以及现在是否合法，但修复前不能直接表达三个与候选后续价值有关的公开差异：来源模式、当前来源是否强制耗尽、是否已有重复选择排队。mask只表达当前能选什么，不能替代这些后续结算语义。
