...
CONTRACT_PATH = Path(__file__).with_name("enemy-potion-contract.json")
...
def normalize_observation(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for enemy in observation["enemies"]:
        ids, valid = enemy.get("intent_history"), enemy.get("intent_history_valid")
        if not isinstance(ids, list) or not isinstance(valid, list) or len(ids) != 3 or len(valid) != 3:
            raise ValueError("敌人意图历史必须有三个位置")
        for move, exists in zip(ids, valid):
            if type(move) is not int or move < 0 or type(exists) is not bool or exists != (move != 0):
                raise ValueError("敌人意图编号与有效性不一致")
...
class EnemyPotionBattleEnv:
    ...
    def __init__(self, max_actions: int = 512, *, backend: Any = None) -> None:
        ...
        if not hasattr(module, "EnemyPotionBattleEnv"):
            raise RuntimeError("扩展缺少EnemyPotionBattleEnv；请先重建新增适配器")
        if getattr(module, "enemy_potion_contract_sha256", None) != CONTRACT_HASH:
            raise RuntimeError("敌人药水后端契约指纹不匹配，必须重新构建")
        self._env = module.EnemyPotionBattleEnv(limit)
    ...
    def reset(self, scene: Mapping[str, Any], seed: int, *, diagnostic: bool = False, purpose: str = "development") -> dict[str, Any]:
        ...
        if purpose != "development" or not diagnostic:
            raise ValueError("扩展目前只准入显式开发诊断，正式训练待集成")
        ...
        if _integer(candidate["act"], "act") not in (1, 2, 3) or candidate["character"] != "IRONCLAD":
            raise ValueError("只支持Ironclad第一至第三幕已准入遭遇")
    ...
    def step(self, action: int):
        ...
        if not self.action_mask()[selected]:
            raise ValueError("动作不在当前合法mask内")
        self._finished = True
