...
def main():
    ...
    from sts.train.m2a import APathTrainer,fingerprint,check_deadline
    from sts.train.corpus_evaluation import evaluate,summarize,strata,paired_comparison
    from sts.models.architectures.m2a_source_context.model import M2aActorCritic as APathActorCritic
    ...
    started=time.monotonic();deadline=started+14400;train_deadline=deadline-3600
    corpus_path=ROOT/'projects/battle-initial-states/corpora/a-v2'
    corpus=ACorpus(corpus_path)
    writer=SummaryWriter(str(out/'tensorboard'/'m2a-source-context-v1'),flush_secs=5)
    config=dict(protocol='experiment-protocol-v1',group=0,device='cuda',num_envs=8,num_steps=128,
                total_transitions=262144,seconds=14400,closure_reserve_seconds=3600,
    ...
    config.update(architecture='m2a_source_context', parameter_count=sum(p.numel() for p in APathActorCritic().parameters()), baseline_run='a-v2-ppo-v1', holdout_scope='previously-used fixed benchmark, not untouched test', initialization='same-seed fresh M0 modules; zero-output source-query residual')
    ...
    for split,n in (('audit-dev',288),('audit-holdout',374)):
    ...
    status=dict(status='starting',pid=os.getpid(),started_at=datetime.now(timezone.utc).isoformat(),
                env_steps=0,iteration=0,budget_seconds=14400,formal_training_started=True,
    ...
    def eval_checkpoint(model,split,label,step):
        ...
        return rows
    try:
        trainer=APathTrainer(corpus_dir=corpus_path,corpus_scope='experiment-protocol-v1',device='cuda')
        ...
        selected=APathActorCritic().cuda()
        checkpoint=torch.load(best_path,map_location='cpu',weights_only=False)
        if checkpoint.get('model_version') != APathActorCritic.model_version:raise ValueError('Selected architecture mismatch')
        if checkpoint['fingerprint']!=fp:raise ValueError('Selected checkpoint fingerprint mismatch')
        selected.load_state_dict(checkpoint['model'])
        before=eval_checkpoint(initial,'audit-holdout','holdout-initial',trainer.env_steps)
        after=eval_checkpoint(selected,'audit-holdout','holdout-selected',trainer.env_steps)
        comparison=paired_comparison(before,after)
        write(out/'holdout-comparison.json',comparison)
        if comparison['status']=='complete':
            writer.add_scalar('holdout/paired_reward_difference',comparison['paired_reward_difference'],trainer.env_steps)
            for i,name in enumerate(('lower','upper')):writer.add_scalar('holdout/reward_ci95_'+name,comparison['reward_difference_ci95'][i],trainer.env_steps)
        if fingerprint(corpus_path)!=fp:raise ValueError('Run fingerprint changed')
        complete=trainer.env_steps==262144 and comparison['status']=='complete'
        refresh('completed' if complete else 'time_limit' if trainer.env_steps<262144 else 'incomplete_evaluation',
                env_steps=trainer.env_steps,iteration=trainer.iteration,step_budget_completed=trainer.env_steps==262144,
                holdout_status=comparison['status'],selected_update=best_update)
