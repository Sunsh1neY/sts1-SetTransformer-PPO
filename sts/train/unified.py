...
from sts.env.unified import UnifiedEnv as ComparisonEnv, load_contract
from sts.env.lightspeed import _load_backend
from sts.env.real_deck import load_batch, sample_scene
from sts.models.unified import UnifiedActorCritic as ComparisonActorCritic, encode, tensor_batch
...
def fingerprint():
    ...
    files = ["sts/train/unified.py", "sts/models/unified.py", "sts/models/unified_fields.py", "sts/env/unified.py", "scripts/run-unified-ppo.py", "eval_seeds.json", "sts/train/comparison.py", "sts/models/comparison.py", "sts/env/comparison.py", "sts/env/real_deck.py",
...
class UnifiedTrainer:
    def __init__(self, kind, group=0, iterations=64, num_envs=8, num_steps=64, device="cpu"):
        ...
        self.model = ComparisonActorCritic(kind, None).to(self.device)
    ...
    def iteration_step(self, deadline=None):
        ...
        for _ in range(self.config["num_steps"]):
            ...
            observations.extend(copy.deepcopy(self.observations))
        ...
        flat = self.batch_tensor(observations)
        ...
        for _ in range(4):
            ...
            for start in range(0, size, size // 4):
                ...
                length = int(mb["valid"].sum(1).max())
                for key in ("features", "kinds", "ids", "valid"):
                    mb[key] = mb[key][:, :length]
    ...
    def save(self, path):
        state = {"schema": "unified-ppo-checkpoint-v1", "config": self.config, "iteration": self.iteration,
    ...
    def load(cls, path):
        ...
        if state.get("schema") != "unified-ppo-checkpoint-v1" or state["fingerprint"] != fingerprint():
