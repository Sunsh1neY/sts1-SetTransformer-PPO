...
from sts.env.comparison import load_contract
from sts.env.real_deck import configured_scene, load_batch
from sts.train.comparison import ComparisonTrainer, check_deadline, evaluate, fingerprint
...
def preflight(output, device):
    ...
    for kind in ("mlp", "set"):
        ...
        for _ in range(4):
            metric, episodes = trainer.iteration_step(deadline=started + 180)
        ...
        trainer.iteration_step(deadline=started + 180)
        restored.iteration_step(deadline=started + 180)
        for k, v in trainer.model.state_dict().items():
            torch.testing.assert_close(v, restored.model.state_dict()[k], rtol=0, atol=0)
    ...
    # 正式四组训练最多分配1100秒，其余留给8次评估、保存及波动。
    slow = max(statistics.mean(r["seconds"] for r in results if r["kind"] == k and r["iteration"] > 1) for k in ("mlp", "set"))
    iterations = max(4, min(512, int(1100 / (4 * slow * 1.35))))
    plan = {"schema": "comparison-ppo-plan-v1", "device": device, "iterations_per_run": iterations,
            "num_envs": 8, "num_steps": 64, "transitions_per_run": iterations * 512,
            "runs": [{"kind": kind, "group": group} for kind in ("mlp", "set") for group in (0, 1)],
            "formal_seconds_limit": 1800, "contract": load_contract(), "fingerprint": fingerprint(),
...
def run(plan_path, output):
    ...
    output.mkdir(parents=True)
    started = time.monotonic()
    deadline = started + plan["formal_seconds_limit"] - 10  # 预留写出中断状态和报告时间。
    write(output / "plan.json", plan)
    write(output / "evaluation-cases.json", cases())
    ...
    try:
        ...
        for group in (0, 1):
            mlp = json.loads((output / f"mlp-{group}/final-evaluation.json").read_text(encoding="utf-8"))
            sets = json.loads((output / f"set-{group}/final-evaluation.json").read_text(encoding="utf-8"))
            report["architecture_comparison"].append({"group": group, "set_minus_mlp": paired(mlp, sets)})
...
if __name__ == "__main__":
    ...
    if args.mode == "preflight":
        preflight(args.output, args.device)
    else:
