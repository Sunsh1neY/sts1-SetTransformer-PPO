...
## F6 最终复核修复（2026-09-13）

价值头接入五维公开上下文，模型升级v4，CARD115不变；补值函数梯度、候选重复/重排、padding污染和空候选测试。撤销+100额外过滤，保留资源异常。修正B05证据边界，未宣称完整输入碰撞。后端构建与补丁反向检查通过；34项定向/恢复/统一实体测试通过。容量保护未放宽，仅因模型契约和后端变化重绑指纹；队列、实例编号及实体容量保持原限制。

## F7 最终验收（2026-09-13）

完整950项CPU测试通过，350局/7897 transition/125次选牌全部自然结束、无异常无截断。32步模型诊断7次真实选牌、3次预算截断，精确下一更新恢复通过。提交补丁从锁定后端隔离索引顺序应用通过。最终可合并结论见card-token-final-review.md；尚未执行合并、推送或正式训练。
