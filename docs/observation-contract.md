# 观测与动作接口契约（V2 已验收；V1 留作历史）

本文固定正式 C++ 后端到后续 Python wrapper 的输入边界。权威依据为
`spec-v6.md` §4、D16、D17、D19、D21、D24、D26、D27；后续实现若改变字段、语义或信息边界，
须先更新决策日志。

版本状态：本文第 1–7 节记录已经实现并通过旧 Gate 1 的 **V1 三类计数版**；第 8 节
记录 2026-09-06 裁定的 **V2 逐牌实体版**。P2–P5 已完成，当前实现以第 8 节为准；
第四类探针、四路径一万配置与性能验收见 [实体基础报告](entity-foundation-report.md)。
下文 P2/P3 的中间结果保留为历史记录，不代表当前仍未完成迁移。
