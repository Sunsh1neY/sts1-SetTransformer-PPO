...
I16 records the completed M0 learning milestone and original ablation proposals. I17 authorized the M2a source-context architecture, I18 its same-budget run, and I19 the continuation to update 256 / 262,144 transitions. The completed dev comparison with M0 found a small reward point-estimate increase but no reliable overall improvement. I20 approves independent Critic variants M3a, C-W128 and C-D4 under the reviewed protocol. The latest saved resume statuses (2026-09-23 02:38 local) are 259,072 / 262,144 for M3a, 196,608 / 262,144 for C-W128 (dev), and 164,864 / 262,144 for C-D4; completion and dev selection remain pending in those snapshots. Preserve M0; these results do not establish Set-versus-MLP or broad generalization.
...
The current architecture-ablation priority is defined by I17-I20, with M0 frozen. Earlier relic and Act 1/2 work remains in [pending plans](docs/plans/README.md) as dated history. I7 still governs any future SlayTheData.7z investigation or A+B initial-state pool preparation: complete and validate the agreed reward, potion and required relic-state work, then freeze contracts and budget before training. A pending plan is not authorization to implement, expand admission, or train. Every required owned relic instance in an admitted scene must be represented with its necessary public state; do not remove unsupported relics merely to admit a scene.

## Execution boundaries

- Preserve user changes, untracked work, ignored experiments, backend patches, and original artifact hashes. Inspect Git state before edits; do not reset, clean, overwrite, or include unrelated work in a commit. The documentation cleanup is authorized in the current checkout, with a local commit and no push.
...
The A-v2 run has closed at update 256. The owner authorized consolidation and retirement of the initial-states worktree on 2026-09-20. The maintained interactive simulator is `tools/battle-console/` in main; preserve original run bytes under `runs/a-v2-ppo-v1/` and the retired worktree snapshot under `runs/worktree-preservation-20260920/`. This does not authorize further training or loosen checkpoint compatibility checks.
