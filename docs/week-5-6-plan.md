...
| 状态 | 任务 | 工程产物 | 完成标准 |
|---|---|---|---|
| [~] | M0 最小切片 PPO+MLP 冒烟 | S1/S2/S3已完成，见 `docs/mlp-smoke-plan.md` | S4/S5待完成；多初始化paired回报差CI与冻结产物；不宣称 Gate 3 |
| [~] | M1 核实卡表来源并筛第一批候选 | 来源记录、10 张数值抽查、候选/排除表 | U3 在决策日志销账；每张候选动作语法可表达 |
| [ ] | M2 定义中等档 reset 与容量契约 | 配置、字段/类别/版本、手牌/目标/牌堆容量 | 按 v6 §4.3.1 审计生成牌和分裂/召唤上界；拒绝超容量，不静默截断 |
...
当前下一执行点为 M0/S4：预先登记多初始化、固定预算和开发配对方案，再验证最小环境
PPO的稳健性。S1/S2/S3工程已完成；S3的262144步首轮训练与300局开发CI见
`docs/ppo-minimal-report.md`。S1–S5 完成后，回到 M1，从正版 JAR
定点取得上述 10 张卡的 base cost / damage / block / magic number，与 HF 原始记录及
锁定后端常量逐项对表；冲突单列。随后按“现有 31 位动作能否完整表达”筛选 30–40 张
正式候选，并在 `docs/decisions.md` 登记来源、卡表和机制批次边界后，才开始 M2/M3
代码扩容。
